...
function getPublicIp() {
    fetch('http://127.0.0.1:5500/datawebDATN.json', {
        method: 'GET',
...
function getWeatherData(city, unit, hourlyorWeek) {
    const apiKey = 'FVX75ESGFYZY79XHD758GS5PV';
...
search.addEventListener('keydown', function(e) {
    ...
    if (e.keyCode == 40) {
        ...
        currentFocus++;
        //lets create a function to adds active suggestion
        addActive(x);
...
function addActive(x) {
    ...
    if (!x) return false;
    removeActive(x);
    //if current focus is nore than the length of suggestion array a make it 0
